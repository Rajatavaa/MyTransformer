...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, seq_length: int, dropout: float):
        ...
        pos = torch.arange(0, seq_length, dtype=torch.float).unsqueeze(
            1
        )  # adding a dimension along column
        ...
        self.register_buffer(
            "pe", pe
        )  # registering the positional encoding matrix as a buffer so that it is not considered a model parameter
...
class MultiHeadAttention(nn.Module):
    ...
    def atten(query, key, value, mask, dropout: nn.Dropout):
        ...
        out = torch.nn.functional.scaled_dot_product_attention(
            query, key, value, attn_mask=mask, dropout_p=dropout_p, is_causal=False
        )
        return out, None
...
class EncoderBlock(nn.Module):
    def __init__(
        self,
        AttentionBlock: MultiHeadAttention,
...
def transformer_work(
    ...
    for _ in range(N):
        ...
        decoder_block = DecoderBlock(
            decoder_block_attention,
            decoder_block_cross_attention,
            feed_forward_block,
            d_model,
            dropout,
        )
